...
> **State as of 2026-09-06, end of day: version 1.1.0, tagged `v1.1.0` and pushed, carrying D182 to D187.** The first strangers played 1.0.1 that morning and could not read it, and the day answered them in six decisions. **D182**, the first hour read back: `legend` and `help colours`, a rule down every key/value grid, a gloss beside every number on `char`, question prompts rendered and fenced for readline, the words in place of the pixel pictures (`rice render picture` brings them back), tables allowed a hundred and twenty columns while prose stays at seventy-six. **D183**, the coach: the tutorial rewritten as lessons that read where the player stands, and inside a network the lesson is the brief itself, so Enter, `now`, `job` and the coach say one thing; four pupils who type nothing but what it says get out clean. **D184**, reach: strict, the author's call; you work on the host you stand on and the ones one hop from it, `scan` alone sees further and Architecture and a hunter program buy the distance; the brief walks toward what it saw; the first night measured before and after (28 and 1 severed of 36, then 27 and none); the ladder's shift recorded as a balance follow-up; two advice loops the new trajectories exposed fixed; every verb, program, implant, signature and help topic swept for the old rule. **D185**, the numbers explained before they are chosen: the origin table shows what you start with, the spend plan is two grids, attributes apart from skills. **D186**, the cold open played for real: `begin` is a run on a hand-built network with Switchboard as the coach in her own voice, a crack that can fail, a Scrapper that wakes, a trace that can fill, a word to read, and a result that reaches the runner made afterwards; its prose seeds what the city has in it. **D187**, the welcome list: three origins first, board ids, the first night's advice, the trace and the money lit in the prompt, an `afterwards` rule after the run card, and `help words`. `validate.py` clean, `test.py` green at **20,442 checks**, the six campaigns clean. Counts (read off the code on 2026-09-06): 15 skills / 30 techniques, 33 traits, 12 origins, 14 icons, 58 implants, 13 weapons, 70 programs, 34 components, 14 drugs, 25 one of a kind, 28 ICE, 63 threads / 215 scenes / 208 decisions, 32 named people, 7 rivals, 204 events, 12 factions, 72 quarters, 102 appearance features, 49 manual topics, 157 commands, 21 lessons, 91 pieces of terminal across 9 axes. **What is next:** the next tester round on the whole first hour; the two balance follow-ups from D184 (the stealth premium on corporate networks under strict reach, and the weakest build's first night); and the two older follow-ups (techniques that decide outcomes; the brief letting go of the verb after the tenth run against a target that remembers you). The paragraph below is the state this day was built on.
>
...
Tagged `v1.1.0` and pushed: the testers' first hour (D182), the coach
(D183), strict reach (D184), the numbers explained (D185), the cold open
as a real run (D186), the welcome list (D187), and the clean-up. `test.py`
green at 20,442; the six campaigns clean.
